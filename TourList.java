...
public class TourList{
    ...
    public void sortTour(){
        ...
        for (int i = 0; i <descendingSortTourPrice().size(); i++)
               System.out.println(descendingSortTourPrice().get(i));
        System.out.println("Sort in ascending order of tour's title: ");
    }
    public ArrayList<Tour> descendingSortTourPrice(){
        ArrayList<Tour> m = new ArrayList<>();
        ArrayList<Tour> n = new ArrayList<>();
        for (Tour v : list) {
            if (v instanceof DomesticTour || v instanceof InternationalTour) {
                m.add(v);
            }
        }
        int flag = 0;
        while (!m.isEmpty()){
            double max = m.get(0).getTourPrice();
            for (int i = 0; i < m.size(); i++)
                if (m.get(i).getTourPrice() > max){
                    max = m.get(i).getTourPrice();
                    flag = i;
                }
            n.add(m.get(flag));
            m.remove(m.get(flag));
